...
<br/><br/>
### 컨트롤러 인터페이스 생성
v3의 컨트롤러 인터페이스는 ModelView를 반환해준다.<br/><br/>
...
### 컨트롤러 인터페이스 생성
파라미터로 paramMap과 model 객체가 전달되고, 결과로 뷰의 이름을 반환해준다.<br/><br/>
...
파라미터로 받은 model에 바로 데이터를 보관하고 viewName을 반환해준다.
```java
//기존 코드
ModelView mv = new ModelView("save-result");
mv.getModel().put("member", member);
...
- **핸들러 어댑터**: 중간에서 어댑터 역할을 한다. 다양한 종류의 컨트롤러를 호출해준다.
